...
if __name__ == "__main__":
    ...
    sender_codebooks = ("cb_1", 2)
    receiver_codebooks = ("cb_2", 4)
    orig_logits, orig_cache = cb_model.run_with_cache(orig_input, return_type="logits")
    new_logits, new_cache = cb_model.run_with_cache(new_input, return_type="logits")
    seq_pos = None

    # Run _path_patch_single
    path_patched_logits = slow_single_path_patch(
        codebook_model=codebook_model,
        orig_input=orig_input,
        new_input=new_input,
        sender_name=sender_codebooks,
        receiver_name=receiver_codebooks,
        seq_pos=seq_pos,
    )
